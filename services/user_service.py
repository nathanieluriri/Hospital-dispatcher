...
def set_new_password(password:PasswordReset):
    ...
    user =find_user_by_email(email=password.email)
    if is_valid==True:
        change_password_service(user_id=user['id'],password=password.password)
        number_of_deleted_fields = db.access_token.delete_many(filter_dict={"user_id":user['id']})
        print("number of deleted access_tokens",number_of_deleted_fields)
